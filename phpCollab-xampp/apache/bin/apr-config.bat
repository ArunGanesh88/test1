...
goto endofperl
@rem ';
#!D:\TAT\phpCollab-xampp\perl\bin\perl.exe
#line 15
use strict;
use warnings;
use Getopt::Long;
...
my ${installbuilddir} = q[D:\TAT\phpCollab-xampp\apache\build];
my ${APR_MAJOR_VERSION} = q[1];
my ${bindir} = q[D:\TAT\phpCollab-xampp\apache\bin];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${APR_SOURCE_DIR} = q[];
my ${includedir} = q[D:\TAT\phpCollab-xampp\apache\include];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${exec_prefix} = q[D:\TAT\phpCollab-xampp\apache];
my ${APR_LIBNAME} = q[libapr-1.lib];
my ${datadir} = q[D:\TAT\phpCollab-xampp\apache];
my ${libdir} = q[D:\TAT\phpCollab-xampp\apache\lib];
my ${CFLAGS} = q[ /nologo /MD /W3 /O2 /D WIN32 /D _WINDOWS /D NDEBUG ];
my ${APR_LIB_TARGET} = q[];
my ${SHELL} = q[C:\WINDOWS\system32\cmd.exe];
my ${CPPFLAGS} = q[];
my ${EXTRA_INCLUDES} = q[];
my ${prefix} = q[D:\TAT\phpCollab-xampp\apache];
